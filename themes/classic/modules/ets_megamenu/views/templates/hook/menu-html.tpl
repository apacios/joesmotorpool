...
<ul class="mm_menus_ul {if isset($mm_config.ETS_MM_CLICK_TEXT_SHOW_SUB) && $mm_config.ETS_MM_CLICK_TEXT_SHOW_SUB} clicktext_show_submenu{/if} {if isset($mm_config.ETS_MM_SHOW_ICON_VERTICAL)&& !$mm_config.ETS_MM_SHOW_ICON_VERTICAL} hide_icon_vertical{/if}">
    ...
    {assign var="parts" value=explode('/', $urls.current_url)}
    {assign var="currentPage" value=$parts|@end}
    {assign var="categoryList" value=Category::getCategories($cookie->id_lang, true, false)}
    ...
    {if false == $isCurrentPage}
        {foreach from=$menus item='menu'}
            {foreach from=$categoryList item=category}
                {if $category.id_parent == $menu.id_category && $category.id_parent != 0}
                    {assign var="currentPage" value=$category.id_parent}
                {/if}
            {/foreach}
        {/foreach}
    {/if}
